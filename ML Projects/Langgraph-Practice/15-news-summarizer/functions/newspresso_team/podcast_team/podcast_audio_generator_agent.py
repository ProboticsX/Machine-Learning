...
def podcast_audio_generator_node(state: AgentState) -> Command[Literal[PODCAST_SUPERVISOR_AGENT]]:
    ...
    podcast_audio_generator_prompt = ChatPromptTemplate.from_messages([
        ...
        ("user", "Here is the podcast transcript file path: {podcast_transcript_file_path}, the category of the podcast: {category} and the output path for the podcast audio: {podcast_audio_file}"),
    ])
    formatted_prompt = podcast_audio_generator_prompt.format(podcast_transcript_file_path=podcast_transcript_file_path, category=category, podcast_audio_file=podcast_audio_file)
