...
def top_headlines_summarizer_node(state: AgentState) -> Command[Literal[TOP_HEADLINES_SUPERVISOR_AGENT, TOP_HEADLINES_CRITIC_AGENT]]:
    ...
    print(state)
    top_headlines_processed_news_file = state["top_headlines_class"]["top_headlines_processed_news_file"]
    top_headlines_critique = ""
    if state.get("top_headlines_class").get("top_headlines_critique") is not None:
        top_headlines_critique = state["top_headlines_class"]["top_headlines_critique"]
    exisiting_top_headlines_summary_json_file = ""
    if state.get("top_headlines_class").get("top_headlines_summary_json_file") is not None:
        exisiting_top_headlines_summary_json_file = state["top_headlines_class"]["top_headlines_summary_json_file"]
    current_top_headlines_critique_count = 0
    if state.get("top_headlines_class").get("top_headlines_critique_count") is not None:
        current_top_headlines_critique_count = state["top_headlines_class"]["top_headlines_critique_count"]
    ...
    system_prompt = f"{role_of_each_top_headlines_worker[TOP_HEADLINES_SUMMARIZER_AGENT]}."+"""
      ...
      Moreover, write the summary to a text file along with the relevant links/sources to the various top headlines in the summary. For example:\n
       ...
       - <Top Headline 3 Summary> \n
           - Relevant links: [Link 1, Link 2, Link 3] \n
    Additionally, write the summary to a json file making sure it's in the valid json format without errors:\n
    ...
    Lastly, push the summary json file to the firebase database as per the category. Please don't push the json file until you fix the validation errors (if any) in the json file.
     """
    prompt = ChatPromptTemplate.from_messages([
        ("system", system_prompt),
        ("user", "Here is the file where you can find the top headlines which you need to summarize: {top_headlines_processed_news_file}, \n the critique of the top headlines (if any): {top_headlines_critique}, \n the existing summary of the top headlines can be found in the json file (if exists): {exisiting_top_headlines_summary_json_file}, \n the category of the top headlines: {category}"),
    ])
    formatted_prompt = prompt.format(top_headlines_processed_news_file=top_headlines_processed_news_file, top_headlines_critique=top_headlines_critique, exisiting_top_headlines_summary_json_file=exisiting_top_headlines_summary_json_file, category=category)
