...
def top_headlines_summarizer_node(state: AgentState) -> Command[Literal[TOP_HEADLINES_SUPERVISOR_AGENT, TOP_HEADLINES_CRITIC_AGENT]]:
    ...
    system_prompt = f"{role_of_each_top_headlines_worker[TOP_HEADLINES_SUMMARIZER_AGENT]}."+"""
      ...
      Moreover, write the summary to a text file along with the relevant links/sources to the various top headlines in the summary. For example:\n
       ...
       - <Top Headline 3 Summary> \n
           - Relevant links: [Link 1, Link 2, Link 3] \n
    Additionally, write the summary to a json file where each json object contains the following fields, make sure there are no invalid control characters in the json file and it is in the valid json format:\n
    ...
    Lastly, push the summary to the firebase database as per the category.
     """
    prompt = ChatPromptTemplate.from_messages([
        ("system", system_prompt),
        ("user", "Here is the file where you can find the top headlines: {top_headlines_processed_news_file}, the critique of the top headlines: {top_headlines_critique}, the existing summary of the top headlines: {exisiting_top_headlines_summary}, the category of the top headlines: {category}"),
    ])
    formatted_prompt = prompt.format(top_headlines_processed_news_file=top_headlines_processed_news_file, top_headlines_critique=top_headlines_critique, exisiting_top_headlines_summary=exisiting_top_headlines_summary, category=category)
