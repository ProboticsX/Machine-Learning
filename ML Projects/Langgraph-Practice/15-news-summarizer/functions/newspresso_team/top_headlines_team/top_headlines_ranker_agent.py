...
def get_score_from_react_agent(system_prompt, top_headlines_summary_json_file, category, date):
    prompt = ChatPromptTemplate.from_messages([
        ("system", system_prompt),
        ("user", "The top headlines can be found here: {top_headlines_summary_json_file}. The category is {category} and the date is {date}."),
    ])
...
def top_headlines_ranker_node(state: AgentState) -> Command[Literal[TOP_HEADLINES_SUPERVISOR_AGENT]]:
    ...
    **Scoring Rules**
    ...
    Moreover, add an entry by the name of the score: "categorical_score" for each of the top headlines you evaluate and save the file back as json.
    """
    get_score_from_react_agent(system_prompt_categorical_score, top_headlines_summary_json_file, category, date)
    ...
    **Scoring Rules**
    ...
    Moreover, add an entry for the given parameter by the name of the score: "source_credibility_score" for each of the top headlines you evaluate and save the file back as json.
    """
    get_score_from_react_agent(system_prompt_source_credibility_score, top_headlines_summary_json_file, category, date)

    system_prompt_time_sensitive_score = f"{role_of_each_top_headlines_worker[TOP_HEADLINES_RANKER_AGENT]}"+"""
    Provides "time_sensitive_score" which is the score assigned to each headline based on how recent the news was. More recent the news and its criticality, the higher the score. \n
    ...
    **Scoring Rules**
    ...
    Moreover, add an entry for the given parameter by the name of the score: "time_sensitive_score" for each of the top headlines you evaluate and save the file back as json.
    """
    get_score_from_react_agent(system_prompt_time_sensitive_score, top_headlines_summary_json_file, category, date)
    ...
    Moreover, add an entry for the given parameter by the name of the score: "ranking_score" for each of the top headlines you evaluate and save the file back as json.
    """
    result = get_score_from_react_agent(system_prompt_ranking_score, top_headlines_summary_json_file, category, date)
