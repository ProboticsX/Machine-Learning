...
def podcast_transcript_finetuner_node(state: AgentState) -> Command[Literal[PODCAST_TRANSCRIPT_SUPERVISOR_AGENT]]:
    ...
    You are tasked with finetuning the podcast transcript. Below are the instructions on how to do it:
     - Assign the name to the podcasters and introduce them in the podcast show: Person1 is Marcus and Person2 is Leslie but don't replace the <Person1> and <Person2> tags in the script.
     - Add human touch to the script by adding interjections, pauses or when the person is thinking.
     - Add other natural human cues like "aah", "um", "like", "you know", etc or when a human repeats some words unintentionally like "yeah, yeah" or "like, like".
