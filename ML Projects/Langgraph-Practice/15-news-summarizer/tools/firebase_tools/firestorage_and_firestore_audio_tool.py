...
class FireStorageAndFireStoreAudioTool:
    ...
    def _delete_existing_audio(self, today_date: str) -> None:
        """
        ...
        try:
            ...
            audio_ref = self.db.collection("news").document("top_headlines").collection("headlines").document(today_date).collection("audio_files")
            ...
            print(f"Deleted existing audio files for {today_date}")
            
        except Exception as e:
            print(f"Error deleting existing audio files: {str(e)}")
    
    def store_audio_file(self, audio_file_path: str, metadata: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            self._delete_existing_audio(today_date)
            
            # Create a unique storage path
            storage_path = f"audio/{today_date}/{file_name}"
            ...
            # Structure: news (collection) -> top_headlines (document) -> headlines (collection) -> today's date (document) -> audio_files (collection)
            metadata_ref = self.db.collection("news").document("top_headlines").collection("headlines").document(today_date).collection("audio_files").document(file_name)
    ...
    def store_audio_directory(self, directory_path: str) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for file_name in os.listdir(directory_path):
                if file_name.lower().endswith(('.mp3', '.wav', '.m4a', '.ogg')):
                    ...
                    result = self.store_audio_file(file_path)
...
if __name__ == "__main__":
    ...
    result = firebase_tools.store_audio_file(audio_path)
    print(result)
    
    # Example of storing all audio files in a directory
    # audio_dir = "../../data/audio"
    # result = firebase_tools.store_audio_directory(audio_dir)
    # print(result) 